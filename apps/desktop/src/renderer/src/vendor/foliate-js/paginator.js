...
export class Paginator extends HTMLElement {
    ...
    getPageInfo() {
        if (!this.#view || this.pages <= 2 || this.#index < 0) return null
        const pageCount = Math.max(1, this.pages - 2)
        const pageIndex = Math.max(0, Math.min(pageCount - 1, this.page - 1))
        return { sectionIndex: this.#index, pageIndex, pageCount }
    }
    setStyles(styles) {
        this.#styles = styles
