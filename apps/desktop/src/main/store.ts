...
export async function saveAgent(input: Partial<Agent>): Promise<DesktopStore> {
  ...
  const username = normalizeUsername(input.username || existing?.username || input.nickname || "agent", "agent");
  const agent: Agent = {
...
function normalizeUsername(value: string, fallback = "me") {
  return value.trim().replace(/^@/, "").replace(/[^a-zA-Z0-9_-]/g, "_").slice(0, 32) || fallback;
}
