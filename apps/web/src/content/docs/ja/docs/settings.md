...
Yomitomoは、macOS と Windows でのアップデート フローをサポートしています。アプリは起動後に自動的に新しいバージョンをチェックし、その後 24 時間ごとにサイレントにチェックします。アップデートが利用可能な場合、読み取りを中断するためのダイアログは開きません。代わりに、上部ナビゲーションに「利用可能なアップデート」バッジが表示され、それをクリックすると、[設定] > [ダウンロードとインストールについて] が開きます。 [設定] > [バージョン情報] の [アップデートの確認] で手動で確認することもできます。アップデートが利用可能な場合は、そのバージョンのリリース ノートが表示されます (アップデート前に Web サイトから取得され、アップデート後にローカルにバンドルされます)。パブリック macOS インストーラーは署名され、公証されています。
