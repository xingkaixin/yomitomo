...
Yomitomo 是一个本地优先的 AI 伴读桌面应用。Electron 桌面端负责网页文章与 EPUB 电子书导入、阅读库、批注、评论、LLM provider 管理、阅读助手和读后卡片生成。
...
packages/core      业务核心逻辑，包括批注、评论、阅读统计、阅读卡片、EPUB 索引和阅读器 DOM 纯逻辑
...
- `packages/core` 放批注、评论、阅读统计、阅读卡片、EPUB 索引和阅读器 DOM 相关纯逻辑。
- `packages/ai` 放 provider 调用、模型输入预算、AI 批注、EPUB 阅读上下文、阅读审议、读后卡片和审核生成。
