...
export function useSearchClearDissolve({
  ...
  const mirrorText = textForSearchMirror(clearing ? clearingText : query);
  ...
  function clearWithDissolve() {
    ...
    elements.mirror.textContent = textForSearchMirror(query);
    elements.glow.style.background = buildSearchClearGlow({
...
function buildSearchClearGlow({
  ...
  text.split(/(\s+)/).forEach((segment) => {
    ...
    if (segment.trim()) {
      ...
      searchClearGlowStops.forEach(([offset, radiusWidth, radiusHeight, alpha]) => {
