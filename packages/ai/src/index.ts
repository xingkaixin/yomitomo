...
export async function runAgentAnnotateStream(
  ...
  const segmentTasks = buildSegmentAnnotationTasks(payload, agent);
  if (segmentTasks.length > 0) {
    await runAgentSegmentAnnotateStream(
      provider,
      agent,
      payload,
      system,
...
async function runAgentSegmentAnnotateStream(
  provider: LlmProvider,
  agent: Agent,
  payload: AgentAnnotatePayload,
  system: string,
