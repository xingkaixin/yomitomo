import { evaluateReadingMemoryHumanRetrieval } from './reading-memory-human-retrieval-evaluation.ts';
import {
  choice,
  entries,
...
export { readingMemoryHumanReviewScope } from './reading-memory-human-review-record.ts';
...
export type ReadingMemoryHumanReviewRecord = ReadingMemoryHumanReviewMetadata & {
  claims: {
    id: string;
    claim: string;
...
export function evaluateReadingMemoryHumanReview(value: unknown) {
  if (typeof value === 'object' && value !== null && Object.hasOwn(value, 'retrieval'))
    return evaluateReadingMemoryHumanRetrieval(value);
...
function readRecord(value: unknown): ReadingMemoryHumanReviewRecord {
  ...
  return {
    ...readHumanReviewMetadata(record),
