...
本文件区分“代码可以安全合入”和“可以向用户开放阅读记忆”。三个入口必须共同通过质量、
隐私、降级和平台验收，不能因为模型能运行或 CI 通过就宣布首期完成。

## 开放状态

发布开关集中在 `apps/desktop/src/reading-memory-release.ts`，当前为 `false`。它同时控制：
...
隔离验收包通过独立构建配置替换开关，不改变正式包。

真实阅读样本和独立人工复核记录未随仓库提供。在补齐下面的人工门槛前，保持开关关闭，
不关闭发布验收 issue，也不宣称 v0.15.0 阅读记忆已经开放。改变开关需要单独审核当次证据，
不得把固定夹具的预期答案作为人工评审结果。
